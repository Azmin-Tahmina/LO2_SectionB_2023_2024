...
/* 
 Filename:    gradesinfo-function.js
...
/**
 * Returns the given numerator divided by the given denominator
 * as a percentage rounded to the number of digits given.
 *
 * @param numerator     Number to be divided
 * @param denominator   Number to divide by
 * @param digits        Digits to display in percentage (default 0)
 * @returns {string}    Percentage as a string to digits given
 */
function asPercent(numerator, denominator, digits = 0) {
...
while (textGrade !== null) { // null will be returned if the user chooses Cancel from the dialog box
    ...
    if (grade < 0 || grade > 100 || Number.isNaN(grade) || isNaN(textGrade)) {
        ...
        window.alert( "Status: " + result
                ...
                + " or " + asPercent(numPasses, numGrades));
...
document.getElementById("output").innerHTML = "Out of " + numGrades + " total grades, "
        ...
        + "The percentage of passing grades was " + asPercent(numPasses, numGrades, 1) + ". "
